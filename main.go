...
import (
	...
	"github.com/vincentvanderweele/wwweeklies-presentations/presentation"
)

func main() {
	storage := presentation.NewStorage()
	errorHandler := presentation.NewErrorTransformer()
...
func respondSwagger(w http.ResponseWriter, r *http.Request) {
	...
	if yamlFile, err = ioutil.ReadFile(path.Join(os.Getenv("GOPATH"), "/src/github.com/vincentvanderweele/wwweeklies-presentations/swagger.yaml")); err != nil {
